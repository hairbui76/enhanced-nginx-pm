...
const internalAccessList = {
		...
		return access
			...
			.then(() => {
				...
				if (typeof data.items !== 'undefined' && data.items) {
					...
					let query = accessListAuthModel.query().delete().where('access_list_id', data.id);
			...
			.then(() => {
				...
				if (typeof data.clients !== 'undefined' && data.clients) {
					...
					let query = accessListClientModel.query().delete().where('access_list_id', data.id);
	...
	getCount: (user_id, visibility) => {
		let query = accessListModel.query().count('id as count').where('is_deleted', 0);
