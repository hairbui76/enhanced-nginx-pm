...
const internalStream = {
	...
	get: (access, data) => {
		...
		return access
			...
			.then((access_data) => {
				let query = streamModel.query().where('is_deleted', 0).andWhere('id', data.id).allowGraph('[owner]').first();
	...
	getAll: (access, expand, search_query) => {
		return access.can('streams:list').then((access_data) => {
			let query = streamModel.query().where('is_deleted', 0).groupBy('id').allowGraph('[owner]').orderBy('incoming_port', 'ASC');
	...
	getCount: (user_id, visibility) => {
		let query = streamModel.query().count('id as count').where('is_deleted', 0);
