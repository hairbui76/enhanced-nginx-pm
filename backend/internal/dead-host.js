...
const internalDeadHost = {
	...
	get: (access, data) => {
		...
		return access
			...
			.then((access_data) => {
				let query = deadHostModel.query().where('is_deleted', 0).andWhere('id', data.id).allowGraph('[owner,certificate]').first();
	...
	getAll: (access, expand, search_query) => {
		return access
			.can('dead_hosts:list')
			.then((access_data) => {
				let query = deadHostModel.query().where('is_deleted', 0).groupBy('id').allowGraph('[owner,certificate]').orderBy('domain_names', 'ASC');
	...
	getCount: (user_id, visibility) => {
		let query = deadHostModel.query().count('id as count').where('is_deleted', 0);
