...
const internalUser = {
	...
	get: (access, data) => {
		...
		return access
			...
			.then(() => {
				let query = userModel.query().where('is_deleted', 0).andWhere('id', data.id).allowGraph('[permissions]').first();
	...
	isEmailAvailable: (email, user_id) => {
		let query = userModel.query().where('email', '=', email.toLowerCase().trim()).where('is_deleted', 0).first();
	...
	getCount: (access, search_query) => {
		return access
			.can('users:list')
			.then(() => {
				let query = userModel.query().count('id as count').where('is_deleted', 0).first();
	...
	getAll: (access, expand, search_query) => {
		return access.can('users:list').then(() => {
			let query = userModel.query().where('is_deleted', 0).groupBy('id').allowGraph('[permissions]').orderBy('name', 'ASC');
